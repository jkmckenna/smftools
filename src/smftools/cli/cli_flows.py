def flow_I(config_path):
    ...
    hmm_adata_basename = basic_analyzed_adata_path.with_suffix("").name + '_hmm.h5ad.gz'
    hmm_adata_path = basic_analyzed_adata_path / hmm_adata_basename
    hmm_backup_dir= hmm_adata_path.parent / 'duplicate_removed_analyzed_adata_I_hmm_accessory_data')
    ...
    if initial_adata:
        ...
        raw_version_available = os.path.exists(initial_adata_path) and os.path.isdir(initial_backup_dir)
    ...
    pp_dir = f"{split_path}/preprocessed"
    ...
    if smf_modality != 'direct':
        ...
        pp_dir = f"{split_path}/preprocessed_duplicates_removed"
        pp_clustermap_dir = f"{pp_dir}/06_clustermaps"
...
############################################### HMM based feature plotting ###############################################
    ...
    # multiqc ###
    if os.path.isdir(f"{split_path}/multiqc"):
        print(f"{split_path}/multiqc" + ' already exists, skipping multiqc')
    else:
        run_multiqc(split_path, f"{split_path}/multiqc")
    ########################################################################################################################