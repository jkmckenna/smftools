...
| `bundle-analysis RUN_ROOT --to BUNDLE_DIR [--stage --generation]` | `data_cmd.py` | `data_bundle_analysis` → `smftools.data.analysis_bundle.bundle_analysis_generations` | Tar every complete, not-yet-bundled published generation into `bundle_root/<stage>/<generation_id>.tar`, idempotent (re-checksums the bundle itself, not the source). Transfer-time file-count fix for the many small partition stores each stage writes; `current.json` stays `data sync`'s job (`TAB-01`). |
| `unbundle-analysis BUNDLE_DIR --to RUN_ROOT [--stage --generation]` | `data_cmd.py` | `data_unbundle_analysis` → `smftools.data.analysis_bundle.unbundle_analysis_generations` | Extract bundles back into an ordinary analysis tree, stage-then-atomic-rename. Verifies the bundle's own checksum before extracting, then every artifact checksum the extracted manifest records (basecall/raw/preprocess only today; other stages report `checksums_verified: false`, a narrower guarantee, not a failure) (`TAB-02`). |
