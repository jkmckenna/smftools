...
sep_BAM_by_BC='/separate_BAM_by_tag.py' 
FORMAT_SMF='/Modkit_extract_to_h5ad.py' 
...
#################################
#### Other variables ####
mod_BAM=''$output_directory'/HAC_5mC_6mA_calls'
...
dorado basecaller $model $POD5_dir --kit-name $barcode_kit -Y --modified-bases 5mC_5hmC 6mA > ''$mod_BAM''$BAM_SUFFIX''
...
modkit summary ''$aligned_sorted_mod_BAM''$BAM_SUFFIX'' --filter-threshold $filter_threshold --mod-thresholds 'm:'$5mC_threshold'' --mod-thresholds 'a:'$m6A_threshold'' --mod-thresholds 'h:'$5hmC_threshold''
...
modkit pileup ''$aligned_sorted_mod_BAM''$BAM_SUFFIX'' $mod_bed_dir --partition-tag BC --only-tabs --filter-threshold $filter_threshold --mod-thresholds 'm:'$5mC_threshold'' --mod-thresholds 'a:'$m6A_threshold'' --mod-thresholds 'h:'$5hmC_threshold''
...
find "$split_dir" -type f -name "*$BAM_SUFFIX" | while IFS= read -r input_file; do
    ...
    modkit extract --filter-threshold $filter_threshold --mod-thresholds 'm:'$5mC_threshold'' --mod-thresholds 'a:'$m6A_threshold'' --mod-thresholds 'h:'$5hmC_threshold'' $input_file null --read-calls $output_tsv
    # Zip the output TSV 
