...
def _adata_for_invert(invert: bool, n_positions: int = 6):
    ...
    adata.layers["hmm_combined_lengths"] = matrix
...
def test_inverted_reference_reverses_column_order(monkeypatch):
    # Regression test: the length-clustermap's "HMM" panel used to hardcode
    # sites=None when building its matrix, ignoring the reordered
    # length_sites already computed for its (correctly reordered) tick
    # labels -- so the panel's x-axis labels looked inverted while the
    # underlying data stayed in physical column order.
