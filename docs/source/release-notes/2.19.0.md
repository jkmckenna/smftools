...
## Results and plots stay in `smftools.analysis`

The ownership boundary is unchanged: `machine_learning` owns models, data, training, and artifacts, while `smftools.analysis` receives results and returns numbers or figures. Two modules are added on the analysis side for canonical ML results — `analysis.compute.ml_results` for fixed-schema summary tables covering metrics, curves, calibration, confusion, folds, balance, history, and attribution, and `analysis.plot.ml_results` for renderers that take a result and an explicit output path. Neither trains, reads a partition store, or decides what a model is.

ML outputs have declared locations: `ml_outputs/` beneath an experiment and `ml/` beneath `project_outputs/`, each holding `datasets/`, `runs/`, `models/`, and `index/`.
